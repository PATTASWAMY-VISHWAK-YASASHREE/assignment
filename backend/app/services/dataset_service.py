...
# Direct access to _dataset_store is NOT thread-safe and
...
async def save_dataset(file: UploadFile) -> DatasetUploadResponse:
    ...
    # Use the underlying file object to check size without reading into memory
    # Seek to end
    file.file.seek(0, 2)
    size = file.file.tell()
    # Seek back to start
    ...
    # Pass the underlying file object directly to pandas to avoid duplication
