...
async def save_dataset(file: UploadFile) -> DatasetUploadResponse:
    # Check file size efficiently by seeking to the end.
    MAX_SIZE = settings.MAX_UPLOAD_SIZE_BYTES

    # file.file is the underlying SpooledTemporaryFile or BytesIO
    file.file.seek(0, 2)
    size = file.file.tell()
    file.file.seek(0)
    ...
    # Pass the file object directly to pandas to avoid reading into memory
    try:
        df = await loop.run_in_executor(
            None, _read_dataframe, filename, file.file
        )
    except Exception as e:
        # Ensure we re-raise exceptions correctly
        raise e
