...
async def save_dataset(file: UploadFile) -> DatasetUploadResponse:
    ...
    # Access the underlying file object to check size
    # Starlette's UploadFile.seek does not support 'whence' argument,
    # so we must access the underlying file directly.
    file.file.seek(0, 2)
    size = file.file.tell()
    file.file.seek(0)
    ...
    # Offload blocking IO/CPU task to a thread pool, passing the file object directly
